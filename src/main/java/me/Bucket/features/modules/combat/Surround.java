...
public class Surround
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue() && !Bucket.moduleManager.isModuleEnabled("Freecam")) {
            if (Surround.mc.world.getBlockState(new BlockPos(Surround.mc.player.getPositionVector())).getBlock() == Blocks.WEB) {
                Bucket.positionManager.setPositionPacket(Surround.mc.player.posX, this.startPos.getY(), Surround.mc.player.posZ, true, true, true);
            } else {
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) {
                ...
                case 1: {
                    ...
                    if (!this.extendMove.getValue().booleanValue() && Bucket.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= this.extender.getValue())
                        continue block6;
    ...
    private boolean check() {
        ...
        switch (this.movementMode.getValue()) {
            ...
            case LIMIT: {
                if (!(Bucket.speedManager.getSpeedKpH() > this.speed.getValue())) break;
                return true;
            }
            case OFF: {
                if (!(Bucket.speedManager.getSpeedKpH() > this.speed.getValue())) break;
        ...
        return Bucket.moduleManager.isModuleEnabled("Freecam") || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }
