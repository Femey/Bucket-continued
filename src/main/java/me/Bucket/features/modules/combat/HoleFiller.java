...
public class HoleFiller
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Bucket.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Bucket.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Bucket.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Bucket.holeManager.getMidSafety());
            }
        }
        object = Bucket.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Bucket.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Bucket.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
