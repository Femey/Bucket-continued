...
import me.Bucket.event.events.TotemPopEvent;
...
public class NoatAura extends Module {
    ...
    public NoatAura() {
        super("NoatAura", "Thx noat <3", Category.COMBAT, true, false, false);
    }
    ...
    public void placePumpkin() {
        List<EntityPlayer> players = mc.world.playerEntities.stream().filter(
                player -> !player.equals(mc.player) && player.getDistance(mc.player) <= 12f && !(player.isDead || player.getHealth() <= 0) && !Bucket.friendManager.isFriend(player.getName())
