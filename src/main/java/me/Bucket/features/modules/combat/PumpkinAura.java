...
import me.Bucket.event.events.Render3DEvent;
...
import me.Bucket.features.modules.player.BlockTweaks;
...
public class PumpkinAura
    ...
    private final Setting<Boolean> antiStep = this.register(new Setting<Boolean>("ThroughWalls", true));
    ...
    public PumpkinAura() {
        super("PumpkinAura", "pumpkin :3", Category.COMBAT, true, false, false);
    }

    @Override
    public void onEnable() {
        this.toAir.clear();
        this.startPos = EntityUtil.getRoundedBlockPos(PumpkinAura.mc.player);
        this.lastHotbarSlot = PumpkinAura.mc.player.inventory.currentItem;
        this.retries.clear();
        if (this.shouldServer()) {
            PumpkinAura.mc.player.connection.sendPacket(new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            PumpkinAura.mc.player.connection.sendPacket(new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module PumpkinAura set Enabled true"));
        }
    }
    ...
    public void onDisable() {
        if (this.shouldServer()) {
            PumpkinAura.mc.player.connection.sendPacket(new CPacketChatMessage("@Serverprefix" + ClickGui.getInstance().prefix.getValue()));
            PumpkinAura.mc.player.connection.sendPacket(new CPacketChatMessage("@Server" + ClickGui.getInstance().prefix.getValue() + "module PumpkinAura set Enabled false"));
            return;
        }
        isPlacing = false;
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.render.getValue().booleanValue() && this.currentPlaceList != null) {
            for (Vec3d vec : this.currentPlaceList) {
                ...
                if (!(PumpkinAura.mc.world.getBlockState(pos).getBlock() instanceof BlockAir)) continue;
    ...
    private void doTrap() {
        ...
        if (this.packet.getValue().booleanValue() && this.airPacket.getValue().booleanValue()) {
            for (Map.Entry<BlockPos, IBlockState> entry : this.toAir.entrySet()) {
                PumpkinAura.mc.world.setBlockState(entry.getKey(), entry.getValue());
            }
            this.toAir.clear();
        }
        if (this.didPlace) {
    ...
    private void placeList(List<Vec3d> list) {
        list.sort((vec3d, vec3d2) -> Double.compare(PumpkinAura.mc.player.getDistanceSq(vec3d2.x, vec3d2.y, vec3d2.z), PumpkinAura.mc.player.getDistanceSq(vec3d.x, vec3d.y, vec3d.z)));
        ...
        for (Vec3d vec3d3 : list) {
            ...
            if (placeability != 3 || this.antiSelf.getValue().booleanValue() && MathUtil.areVec3dsAligned(PumpkinAura.mc.player.getPositionVector(), vec3d3))
                continue;
            this.placeBlock(position);
        }
    }
    ...
    private boolean check() {
        ...
        int pumpSlot = -1;
        if (pumpSlot == -1) {
            pumpSlot = InventoryUtil.findHotbarBlock(BlockPumpkin.class);
        } else {
            pumpSlot = InventoryUtil.findHotbarBlock(BlockPumpkin.class);
        }
        if (this.isOff()) {
        ...
        if (this.disable.getValue().booleanValue() && this.startPos != null && !this.startPos.equals(EntityUtil.getRoundedBlockPos(PumpkinAura.mc.player))) {
            this.disable();
            return true;
        }
        ...
        if (pumpSlot == -1) {
            if (this.switchMode.getValue() != InventoryUtil.Switch.NONE) {
                if (this.info.getValue().booleanValue()) {
                    Command.sendMessage("<" + this.getDisplayName() + "> " + "\u00a7c" + "You are out of Pumpkin.");
                }
                this.disable();
            }
            return true;
        }
        if (PumpkinAura.mc.player.inventory.currentItem != this.lastHotbarSlot && PumpkinAura.mc.player.inventory.currentItem != pumpSlot) {
            this.lastHotbarSlot = PumpkinAura.mc.player.inventory.currentItem;
        }
        this.switchItem(true);
        this.isSneaking = EntityUtil.stopSneaking(this.isSneaking);
        this.target = this.getTarget(this.targetRange.getValue(), this.targetMode.getValue() == TargetMode.UNTRAPPED);
        return this.target == null || Bucket.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && PumpkinAura.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockPumpkin.class);
    }

    private EntityPlayer getTarget(double range, boolean trapped) {
        EntityPlayer target = null;
        double distance = Math.pow(range, 2.0) + 1.0;
        for (EntityPlayer player : PumpkinAura.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || this.pattern.getValue() == Pattern.STATIC && trapped && EntityUtil.isTrapped(player, this.antiScaffold.getValue(), this.antiStep.getValue(), this.legs.getValue(), this.platform.getValue(), this.antiDrop.getValue(), this.face.getValue()) || this.pattern.getValue() != Pattern.STATIC && trapped && EntityUtil.isTrappedExtended(this.extend.getValue(), player, this.antiScaffold.getValue(), this.antiStep.getValue(), this.legs.getValue(), this.platform.getValue(), this.antiDrop.getValue(), this.raytrace.getValue(), this.noScaffoldExtend.getValue(), this.face.getValue()) || EntityUtil.getRoundedBlockPos(PumpkinAura.mc.player).equals(EntityUtil.getRoundedBlockPos(player)) && this.antiSelf.getValue().booleanValue() || Bucket.speedManager.getPlayerSpeed(player) > this.speed.getValue())
                continue;
            if (target == null) {
                target = player;
                distance = PumpkinAura.mc.player.getDistanceSq(player);
                continue;
            }
            if (!(PumpkinAura.mc.player.getDistanceSq(player) < distance)) continue;
            target = player;
            distance = PumpkinAura.mc.player.getDistanceSq(player);
        }
        return target;
    ...
    private void placeBlock(BlockPos pos) {
        if (this.placements < this.blocksPerPlace.getValue() && PumpkinAura.mc.player.getDistanceSq(pos) <= MathUtil.square(this.range.getValue()) && this.switchItem(false)) {
            isPlacing = true;
            if (this.airPacket.getValue().booleanValue() && this.packet.getValue().booleanValue()) {
                this.toAir.put(pos, PumpkinAura.mc.world.getBlockState(pos));
            }
    ...
    private boolean switchItem(boolean back) {
        boolean[] value = InventoryUtil.switchItem(back, this.lastHotbarSlot, this.switchedItem, this.switchMode.getValue(), InventoryUtil.findHotbarBlock(BlockPumpkin.class) != -1 ? BlockPumpkin.class : BlockPumpkin.class);
