...
import me.Bucket.features.setting.EnumConverter;
...
public class Offhand
    ...
    public Setting<Bind> PumpkinBind = this.register(new Setting<Bind>("Pumpkins", new Bind(-1)));
    public Setting<Float> PumpkinHealth = this.register(new Setting<Float>("W-Health", Float.valueOf(13.0f), Float.valueOf(0.1f), Float.valueOf(36.0f)));
    public Setting<Float> PumpkinHoleHealth = this.register(new Setting<Float>("W-H-Health", Float.valueOf(8.0f), Float.valueOf(0.1f), Float.valueOf(36.0f)));
    ...
    public Setting<Boolean> cyclePumpkins = this.register(new Setting<Object>("CyclePumpkins", Boolean.valueOf(false), v -> this.type.getValue() == Type.OLD));
    ...
    public boolean holdingPumpkin = false;
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState()) {
            if (this.type.getValue() == Type.NEW) {
                ...
                if (this.PumpkinBind.getValue().getKey() == Keyboard.getEventKey()) {
                    if (this.mode == Mode.PumpkinS) {
                        this.setSwapToTotem(!this.isSwapToTotem());
                    } else {
                        this.setSwapToTotem(false);
                    }
                    this.setMode(Mode.PumpkinS);
                }
                ...
                if (this.cycleKey.getValue().getKey() == Keyboard.getEventKey()) {
                    ...
                    if (newMode == Mode2.OBSIDIAN && !this.cycleObby.getValue().booleanValue() || newMode == Mode2.PumpkinS && !this.cyclePumpkins.getValue().booleanValue()) {
    ...
    public void doOffhand() {
        ...
        if (this.type.getValue() == Type.NEW) {
            ...
            this.holdingPumpkin = InventoryUtil.isBlock(Offhand.mc.player.getHeldItemOffhand().getItem(), BlockPumpkin.class);
            ...
            if (this.currentMode == Mode2.PumpkinS || this.currentMode == Mode2.OBSIDIAN) {
                ...
                this.Pumpkins = Offhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockPumpkin.class)).mapToInt(ItemStack::getCount).sum();
                if (this.holdingPumpkin) {
                    this.Pumpkins += Offhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockPumpkin.class)).mapToInt(ItemStack::getCount).sum();
                }
            }
            this.doSwitch();
        }
    ...
    public void doSwitch() {
        ...
        switch (this.currentMode) {
            ...
            case PumpkinS: {
                if (this.Pumpkins <= 0 || this.holdingPumpkin) break;
                this.lastPumpkinSlot = InventoryUtil.findInventoryBlock(BlockPumpkin.class, false);
                int lastSlot = this.getLastSlot(currentOffhandItem, this.lastPumpkinSlot);
                this.putItemInOffhand(this.lastPumpkinSlot, lastSlot);
                break;
            }
            case OBSIDIAN: {
    ...
    private int getLastSlot(Item item, int slotIn) {
        ...
        if (InventoryUtil.isBlock(item, BlockPumpkin.class)) {
            return this.lastPumpkinSlot;
        }
        if (item == Items.AIR) {
            return -1;
        }
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !Bucket.friendManager.isFriend(e) && Offhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }
    ...
    private boolean isItemInOffhand() {
        switch (this.mode) {
            ...
            case PumpkinS: {
                return Offhand.mc.player.getHeldItemOffhand().getItem() instanceof ItemBlock && ((ItemBlock) Offhand.mc.player.getHeldItemOffhand().getItem()).block == Blocks.PUMPKIN;
            }
        }
        return false;
    }
    ...
    private boolean isHeldInMainHand() {
        switch (this.mode) {
            ...
            case PumpkinS: {
                return Offhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && ((ItemBlock) Offhand.mc.player.getHeldItemMainhand().getItem()).block == Blocks.PUMPKIN;
            }
        }
        return false;
    }
    ...
    private float getHealth() {
        ...
        return this.PumpkinHealth.getValue().floatValue();
    }
    ...
    private float getHoleHealth() {
        ...
        return this.PumpkinHoleHealth.getValue().floatValue();
    }
    ...
    int getStackSize() {
        ...
        } else if (this.mode == Mode.PumpkinS) {
            for (int i = 45; i > 0; --i) {
                if (!(Offhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock) Offhand.mc.player.inventory.getStackInSlot(i).getItem()).block != Blocks.PUMPKIN)
                    continue;
                size += Offhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else {
    ...
    public void setMode(Mode2 mode) {
        this.currentMode = this.currentMode == mode ? Mode2.TOTEMS : (this.cycle.getValue() == false && this.crystalToTotem.getValue() != false && (this.currentMode == Mode2.CRYSTALS || this.currentMode == Mode2.OBSIDIAN || this.currentMode == Mode2.PumpkinS) && mode == Mode2.GAPPLES ? Mode2.TOTEMS : mode);
    }
