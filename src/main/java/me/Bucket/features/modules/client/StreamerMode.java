...
import me.Bucket.util.TextUtil;
...
public class StreamerMode
    ...
    public void onUpdate() {
        if (this.window != null) {
            ...
            drawInfo.add("TPS: " + Bucket.serverManager.getTPS());
            drawInfo.add("Ping: " + Bucket.serverManager.getPing() + "ms");
            drawInfo.add("Speed: " + Bucket.speedManager.getSpeedKpH() + "km/h");
            ...
            String text = Bucket.rotationManager.getDirection4D(false);
            drawInfo.add("");
            drawInfo.add(text);
            drawInfo.add(coordinates);
            drawInfo.add("");
            for (Module module : Bucket.moduleManager.sortedModules) {
                String moduleName = TextUtil.stripColor(module.getFullArrayString());
                drawInfo.add(moduleName);
            }
            drawInfo.add("");
            for (PotionEffect effect : Bucket.potionManager.getOwnPotions()) {
                String potionText = TextUtil.stripColor(Bucket.potionManager.getColoredPotionString(effect));
