...
import me.Bucket.event.events.Render2DEvent;
import me.Bucket.features.modules.Module;
import me.Bucket.features.modules.misc.ToolTips;
import me.Bucket.manager.TextManager;
...
public class HUD
    ...
    private static final ResourceLocation codHitmarker = new ResourceLocation("Bucket", "cod_hitmarker");
    public static final SoundEvent COD_EVENT = new SoundEvent(codHitmarker);
    private static final ResourceLocation csgoHitmarker = new ResourceLocation("Bucket", "csgo_hitmarker");
    ...
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "Bucket.cum"));
    ...
    public void onUpdate() {
        for (final Module module : Bucket.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case Bucket: {
                this.renderer.drawString("Bucket" + (this.modeVer.getValue() ? " v1.1.4" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() ? " v1.1.4" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Bucket.moduleManager.alphabeticallySortedModules.size() : Bucket.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Bucket.moduleManager.alphabeticallySortedModules.get(k) : Bucket.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "§7" + ((module.getDisplayInfo() != null) ? (" [§f" + module.getDisplayInfo() + "§7" + "]") : "");
                    final Color moduleColor = Bucket.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Bucket.moduleManager.alphabeticallySortedModules.size() : Bucket.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Bucket.moduleManager.alphabeticallySortedModules.get(Bucket.moduleManager.alphabeticallySortedModules.size() - 1 - k) : Bucket.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "§7" + ((module.getDisplayInfo() != null) ? (" [§f" + module.getDisplayInfo() + "§7" + "]") : "");
                    final Color moduleColor = Bucket.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + "§f" + Bucket.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Bucket.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? Bucket.potionManager.getPotionString(effect) : Bucket.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + "§f" + Bucket.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + "§f" + Bucket.serverManager.getTPS();
            ...
            final String fpsText = grayString + "FPS " + "§f" + Minecraft.debugFPS;
            final String text = grayString + "Ping " + "§f" + Bucket.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue()) {
            Bucket.notificationManager.handleNotifications(height - (k + 16));
        } else {
            Bucket.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        final String text4 = (this.direction.getValue() ? (Bucket.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case CHRISTMAS: {
                text = text + "Merry Christmas " + HUD.mc.player.getDisplayNameString() + " :3)";
                break;
            }
            case LONG: {
                text = text + "Welcome to Bucket" + HUD.mc.player.getDisplayNameString() + " :3)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (Bucket.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? "§7" : "§c") + "Server not responding: " + MathUtil.round(Bucket.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
