...
public class Managers
    ...
    public Setting<String> command = this.register(new Setting<String>("Command", "Bucket"));
    ...
    @Override
    public void onLoad() {
        Bucket.commandManager.setClientMessage(this.getCommandMessage());
    }
        ...
        if (event.getStage() == 2) {
            ...
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    Bucket.holeManager.settingChanged();
                }
                Bucket.commandManager.setClientMessage(this.getCommandMessage());
            }
        }
