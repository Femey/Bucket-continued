...
public class Speed
    ...
    private boolean shouldReturn() {
        return Bucket.moduleManager.isModuleEnabled("Freecam") || Bucket.moduleManager.isModuleEnabled("Phase") || Bucket.moduleManager.isModuleEnabled("ElytraFlight") || Bucket.moduleManager.isModuleEnabled("Strafe") || Bucket.moduleManager.isModuleEnabled("Flight");
    }
        ...
        if (EntityUtil.isEntityMoving(Speed.mc.player) && !Speed.mc.player.collidedHorizontally && !BlockUtil.isBlockAboveEntitySolid(Speed.mc.player) && BlockUtil.isBlockBelowEntitySolid(Speed.mc.player)) {
            ...
            if (Speed.mc.player.posY >= this.startY + this.bounceHeight) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    Bucket.timerManager.setTimer(1.0f);
                }
            }
            if (Speed.mc.player.posY == this.startY) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    if (rnd) {
                        Bucket.timerManager.setTimer(1.3f);
                    } else {
                        Bucket.timerManager.setTimer(1.0f);
                    }
                }
