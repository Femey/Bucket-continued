...
public class ChinaText extends Module {
    public ChinaText() {super("ChinaText", "Sends The Annoying Lag Text", Category.TROLL, true, false, false);}

    public Setting<Boolean> friends = this.register(new Setting<Boolean>("Friends", false));

    public static ChinaText INSTANCE = new ChinaText();

    protected static final String China =
    ...
    public void onPop(EntityPlayer event) {
        if (ChinaText.fullNullCheck() || ChinaText.mc.player.equals(event.getName())) return;
        TotemPopCount++;
        if (TotemPopCount == 1) {
            if (!Bucket.friendManager.isFriend((event.getName()))) {
                if (this.enabled.getValue()) {
                    mc.player.connection.sendPacket(new CPacketChatMessage("/msg " + event.getName() + " " + China));
                } else if (friends.getValue()){
                    if (this.enabled.getValue()) {
                        mc.player.connection.sendPacket(new CPacketChatMessage("/msg " + event.getName() + " " + China));
                    }
                }
