...
import me.Bucket.event.events.DeathEvent;
...
public class AutoGG
        extends Module {
    private static final String path = "Bucket/autogg.txt";
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (event.getTarget() instanceof EntityPlayer && !Bucket.friendManager.isFriend(event.getEntityPlayer())) {
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(AutoGG.mc.world) instanceof EntityPlayer && !Bucket.friendManager.isFriend((EntityPlayer) packet.getEntityFromWorld(AutoGG.mc.world))) {
