package me.Bucket.features.gui.components;

import me.Bucket.features.gui.components.items.Item;
import me.Bucket.features.gui.components.items.buttons.Button;
...
import me.Bucket.features.gui.components.items.buttons.ModuleButton;
...
public class Component
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.open) {
            ...
            if (ClickGui.getInstance().outline.getValue().booleanValue()) {
                if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
                    ...
                    GlStateManager.shadeModel(7425);
                    GL11.glBegin(2);
                    Color outlineColor = ClickGui.getInstance().colorSync.getValue() != false ? new Color(Colors.INSTANCE.getCurrentColorHex()) : new Color(Bucket.colorManager.getColorAsIntFullAlpha());
        ...
        Bucket.textManager.drawStringWithShadow(this.getName(), (float) this.x + 3.0f, (float) this.y - 4.0f - (float) BucketGui.getClickGui().getTextOffset(), -1);
        if (this.open) {
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0 && this.isHovering(mouseX, mouseY)) {
            ...
            BucketGui.getClickGui().getComponents().forEach(component -> {
                if (component.drag) {
