...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? Bucket.colorManager.getColorWithAlpha(Bucket.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Bucket.colorManager.getColorWithAlpha(Bucket.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515));
        }
        if (this.isListening) {
            Bucket.textManager.drawStringWithShadow(this.currentString.getString() + Bucket.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) BucketGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            Bucket.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " " + "\u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) BucketGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
