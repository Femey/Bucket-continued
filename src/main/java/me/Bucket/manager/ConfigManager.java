...
import me.Bucket.features.modules.render.XRay;
...
public class ConfigManager
    ...
    public String config = "Bucket/config/";
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Bucket.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Bucket").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("Bucket/" + name + "/")) ? "Bucket/" + name + "/" : "Bucket/config/";
    ...
    public void saveConfig(String name) {
        ...
        Bucket.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Bucket/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Bucket", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Bucket.moduleManager.modules);
        this.features.add(Bucket.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Bucket.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Bucket.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
